...
def seed_video_rows_pending(project_id: str) -> None:
    """Seed video_assets rows as 'pending' (not queued) so the videos_control
    page can display each shot without triggering any rendering."""
    with _db() as conn, conn.cursor() as cur:
        cur.execute("SELECT styled_timeline FROM projects WHERE id=%s", (project_id,))
        row = cur.fetchone()
    if not row or not row.get("styled_timeline"):
    ...
    with _db() as conn, conn.cursor() as cur:
        for shot in timeline:
            ...
            cur.execute(
                ...
                ON CONFLICT (project_id, shot_index) DO UPDATE
                   SET status = CASE
                       WHEN video_assets.status IN ('ready') THEN video_assets.status
                       ELSE 'pending'
                   END,
                   updated_at = NOW()
                """,
...
def kick_single_video(project_id: str, shot_index: int) -> None:
    """Kick rendering for one video clip from videos_control page."""
    with _db() as conn, conn.cursor() as cur:
        cur.execute("SELECT styled_timeline FROM projects WHERE id=%s", (project_id,))
        row = cur.fetchone()
    if not row or not row.get("styled_timeline"):
    ...
    if shot is None:
        return
    _update_video(project_id, shot_index, "rendering")
    _VIDEO_EXECUTOR.submit(_render_video, project_id, shot)


def kick_all_pending_videos(project_id: str, force: bool = False) -> None:
    """Kick rendering for all pending (or failed) video clips from videos_control page."""
    with _db() as conn, conn.cursor() as cur:
        cur.execute("SELECT styled_timeline FROM projects WHERE id=%s", (project_id,))
        row = cur.fetchone()
        cur.execute(
            "SELECT shot_index, status FROM video_assets WHERE project_id=%s",
            (project_id,),
        )
        existing = {r["shot_index"]: r["status"] for r in (cur.fetchall() or [])}
    if not row or not row.get("styled_timeline"):
        return
    timeline = list(row["styled_timeline"])
    for shot in timeline:
        idx = shot.get("shot_index") or shot.get("timeline_index")
        current_status = existing.get(idx, "pending")
        if force or current_status not in ("ready", "rendering"):
            _update_video(project_id, idx, "rendering")
            _VIDEO_EXECUTOR.submit(_render_video, project_id, shot)
