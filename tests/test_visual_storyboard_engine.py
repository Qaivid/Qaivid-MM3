...
# ---------------------------------------------------------------------------
# Tests 9 & 10 – Intensity-based word substitution in motion prompts
# ---------------------------------------------------------------------------
#
# When intensity >= 0.8 the engine swaps "slow" -> "moderately dynamic" and
# "gentle" -> "purposeful" inside the chosen template. When intensity < 0.3
# the reverse swap is enforced. These tests pin down that behaviour so a
# refactor cannot silently break cinematic pacing guidance.
# ---------------------------------------------------------------------------

# (mode, frame_idx) pairs whose original template contains the word "slow".
_SLOW_TEMPLATE_CASES = [
    ("face", 0),         # "slow push-in toward face..."
    ("body", 0),         # "slow pan following body weight shift..."
    ("body", 3),         # "slow orbit around seated figure..."
    ("environment", 0),  # "slow drift through empty space..."
    ("environment", 3),  # "slow tilt down from sky..."
    ("symbolic", 0),     # "slow dolly toward object..."
    ("symbolic", 3),     # "slow pull back from detail..."
    ("macro", 2),        # "slow push-in on surface detail"
]


@pytest.mark.parametrize("mode,frame_idx", _SLOW_TEMPLATE_CASES)
def test_high_intensity_removes_slow_from_motion_prompt(mode: str, frame_idx: int):
    """
    For every (mode, frame_idx) whose original template contains "slow", a
    high-intensity (>= 0.8) shot's motion_prompt must NOT contain that word —
    it should have been substituted with "moderately dynamic".
    """
    engine = VisualStoryboardEngine()
    original_template = engine._MOTION_TEMPLATES[mode][frame_idx]
    assert "slow" in original_template, (
        f"Test fixture stale: template ({mode}, {frame_idx}) no longer contains 'slow'."
    )

    shot = _make_shot(1, expression_mode=mode, intensity=0.9)
    prompt = engine._build_motion_prompt(mode, frame_idx, shot)

    assert "slow" not in prompt, (
        f"High-intensity motion_prompt for ({mode}, frame_idx={frame_idx}) still "
        f"contains 'slow': '{prompt}'"
    )
    assert "moderately dynamic" in prompt, (
        f"Expected 'moderately dynamic' substitution in high-intensity prompt "
        f"for ({mode}, frame_idx={frame_idx}), got: '{prompt}'"
    )


# All expression modes whose template set contains either "slow" or "gentle"
# (i.e. words eligible for the high-intensity substitution path). This is
# every mode currently defined.
_LOW_INTENSITY_MODES = ["face", "body", "environment", "symbolic", "macro"]


@pytest.mark.parametrize("mode", _LOW_INTENSITY_MODES)
def test_low_intensity_motion_prompt_never_contains_moderately_dynamic(mode: str):
    """
    A low-intensity (< 0.3) shot must never produce a motion_prompt containing
    "moderately dynamic" — that wording is reserved for the high-intensity
    branch and the low-intensity branch must enforce "slow"/"gentle" wording.
    Checked across every frame_idx in each mode's template set.
    """
    engine = VisualStoryboardEngine()
    templates = engine._MOTION_TEMPLATES[mode]

    for frame_idx in range(len(templates)):
        shot = _make_shot(1, expression_mode=mode, intensity=0.1)
        prompt = engine._build_motion_prompt(mode, frame_idx, shot)

        assert "moderately dynamic" not in prompt, (
            f"Low-intensity motion_prompt for ({mode}, frame_idx={frame_idx}) "
            f"unexpectedly contains 'moderately dynamic': '{prompt}'"
        )
        assert "purposeful" not in prompt, (
            f"Low-intensity motion_prompt for ({mode}, frame_idx={frame_idx}) "
            f"unexpectedly contains 'purposeful': '{prompt}'"
        )
