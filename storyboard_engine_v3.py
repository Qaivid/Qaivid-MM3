...
def _call2_system_prompt() -> str:
    return (
        ...
        "━━ VISUAL IDEAS — USE THESE FIRST ━━\n"
        "Each scene in the user prompt contains a 'visual_ideas' list — concrete "
        "one-sentence shot ideas generated by Call 1. These are your PRIMARY source "
        "for action_intents. Distribute these ideas across the scene's shots, adapting "
        "the phrasing to the lyric timing. You may rephrase, combine, or expand them, "
        "but you must draw from them — do NOT invent something disconnected, and do "
        "NOT copy the 'purpose' text. If a scene has 3 visual_ideas and 5 shots, "
        "elaborate on them; never fall back to the purpose as an action_intent.\n\n"
        ...
        "   Draw from the scene's visual_ideas — NEVER copy or paraphrase the "
        "   scene's purpose field.\n"
...
def _call2_user_prompt(scenes: List[Dict[str, Any]], audio_duration: float,
    ...
    scenes_block = json.dumps(
        [{
            ...
            # valid_realizations from Call 1 — concrete visual ideas to draw shots from.
            # Use these as inspiration for action_intents instead of restating the purpose.
            "visual_ideas":        (s.get("valid_realizations") or [])[:6],
        } for s in scenes],
