...
def advance_brief(project_id: str):
    ...
    try:
        with db() as conn:
            ...
            briefs["approved"] = True
            brain.write("creative_briefs", briefs)
            brain.save(conn)
            conn.commit()
        app.logger.info(
            "ProjectBrain: marked creative brief approved for project=%s (%d scenes)",
            project_id, len(briefs.get("scenes") or []),
        )
    except Exception:
...
def regenerate_brief(project_id: str):
    ...
    # Clear the prior brief snapshot so the new brief review starts clean.
    # v2 brief is regenerated end-to-end from storyboard scenes.
    cb.pop("chosen", None)
    cb.pop("scenes", None)
    ...
    # Note: do NOT clear styled_timeline here — regenerating the brief does
    # not require re-running the storyboard.
    with db() as conn, conn.cursor() as cur:
        cur.execute(
...
def rerun_from_stage(project_id: str, target_stage: str):
    ...
    with db() as conn, conn.cursor() as cur:
        ...
        if target_stage == "audio_review":
            ...
            # Storyboard now runs BEFORE brief — clear the legacy
            # creative_brief snapshot so the brief is regenerated fresh
            # from the new storyboard scenes.
            cp.pop("creative_brief", None)
            cur.execute(
    ...
    if target_stage == "audio_review":
        ...
        # Brief reads everything it needs from the brain (storyboard +
        # narrative + context + style); no overrides to thread through.
        kick_stage_brief(project_id, {})
        flash(f"Re-running Creative Brief. You'll land on {label} when ready.", "info")
    elif target_stage == "storyboard_review":
        overrides = {
...
def advance_stage_3(project_id: str):
    """User approved the storyboard — kick the Creative Brief (Stage 6).

    Storyboard = possibilities. Creative Brief = controlled selection.
    The brief LLM picks one realization per scene and locks the per-scene
    execution intent, then parks at `creative_brief_review` for user yes
    or regenerate. Reference plates are generated only after brief approval.
    """
    project = _get_project(project_id, current_user()["id"])
    if not project:
        abort(404)

    # Atomic gate: only kick brief if project is parked at storyboard_review.
    with db() as conn, conn.cursor() as cur:
        cur.execute(
            "UPDATE projects SET status=%s, stage=%s, error=NULL, "
            "       updated_at=NOW() "
    ...
    # Stage 6 — Creative Brief (controlled selection over storyboard scenes).
    # Brief reads everything it needs from the brain; no overrides to thread.
    kick_stage_brief(project_id, {})
    return redirect(url_for("project_detail", project_id=project_id))
